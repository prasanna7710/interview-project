export interface ParsedResumeData {
  ...
  experiences: Array<{ company: string; role: string; duration: string; responsibilities: string[] }>;
  educations: Array<{ degree: string; institution: string; year: string; details?: string }>;
}
...
export async function parseResumeWithAI(rawText: string, resumeId?: string, filename?: string): Promise<ParsedResumeData> {
 ...
 */
function smartResumeFallback(rawText: string): ParsedResumeData {
  const text = rawText || '';
  ...
  const projectsList: Array<{ title: string; description: string; technologies: string[]; link?: string }> = [];
  ...
  const experiencesList: Array<{ company: string; role: string; duration: string; responsibilities: string[] }> = [];
  ...
  const educationsList: Array<{ degree: string; institution: string; year: string; details?: string }> = [];
...
function generateSmartQuestionsFallback(
  type: string,
  difficulty: string,
  count: number,
  resumeData?: ParsedResumeData | null
...
function evaluateAnswerSmartFallback(questionText: string, userAnswer: string): AnswerEvaluationResult {
  ...
  if (relevance >= 80) strengths.push('Directly addressed the question asked.');
  if (communication >= 80) strengths.push('Clear expression and readable structure.');
  if (matchedKeywords.length > 2) strengths.push('Used relevant technical terminology and practical concepts.');
  if (strengths.length === 0) strengths.push('Attempted the response directly.');

  if (wordCount < 30) improvements.push('Expand your response using the STAR method (Situation, Task, Action, Result).');
  if (matchedKeywords.length < 2) improvements.push('Include specific technical details, algorithms, or tool choices.');
  if (improvements.length === 0) improvements.push('Practice summarizing key trade-offs at the end of your response.');
