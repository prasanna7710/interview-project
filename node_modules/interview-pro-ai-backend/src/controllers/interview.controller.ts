...
import { generateInterviewQuestions, generateFollowUpQuestion, evaluateAnswer } from '../services/ai.service';
...
export async function createInterview(req: AuthRequest, res: Response) {
  try {
    ...
    if (resumeId) {
      resumeData = await prisma.resume.findFirst({
...
export async function getInterviewById(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.userId;
    const { id } = req.params;
...
export async function submitAnswer(req: AuthRequest, res: Response) {
  try {
    ...
    if (enableFollowUps && !question.isFollowUp && Math.random() > 0.4) {
      const followUpText = await generateFollowUpQuestion(question.questionText, answerText.trim());
      
      const newQuestionCount = await prisma.interviewQuestion.count({ where: { interviewId } });
      
      followUpQuestion = await prisma.interviewQuestion.create({
        data: {
          interviewId,
