...
exports.setupCodingTest = setupCodingTest;
exports.getCodingTest = getCodingTest;
exports.runCode = runCode;
exports.submitQuestion = submitQuestion;
exports.finishCodingTest = finishCodingTest;
exports.getCodingHistory = getCodingHistory;
const database_1 = require("../config/database");
const codeExecution_service_1 = require("../services/codeExecution.service");
...
async function setupCodingTest(req, res) {
    try {
        const userId = req.user?.userId;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const { language = 'Python', difficulty = 'Intermediate', questionCount = 5, durationMinutes = 30, } = req.body;
        const latestResume = await database_1.prisma.resume.findFirst({
            where: { userId },
        ...
        const codingTest = await database_1.prisma.codingTest.create({
            data: {
                userId,
                language,
                difficulty,
...
async function getCodingTest(req, res) {
    try {
        const userId = req.user?.userId;
        const { id } = req.params;
        const codingTest = await database_1.prisma.codingTest.findFirst({
            where: { id, userId },
            include: {
                questions: true,
                submissions: true,
...
async function runCode(req, res) {
    try {
        ...
        const testQuestion = await database_1.prisma.codingTestQuestion.findFirst({
        ...
        const executionResult = await (0, codeExecution_service_1.executeCodeSafely)(code, language || 'Python', visibleTestCases.length > 0 ? visibleTestCases : allTestCases);
...
async function submitQuestion(req, res) {
    try {
        ...
        const executionResult = await (0, codeExecution_service_1.executeCodeSafely)(code, language || 'Python', allTestCases);
        const submission = await database_1.prisma.codingSubmission.create({
            data: {
                testId,
                questionId,
                code,
...
async function finishCodingTest(req, res) {
    try {
        const userId = req.user?.userId;
        const { testId } = req.params;
        ...
        Object.values(latestSubmissionsByQuestion).forEach((sub) => {
        ...
        const updated = await database_1.prisma.codingTest.update({
            where: { id: testId },
            data: {
                status: 'completed',
                score,
...
async function getCodingHistory(req, res) {
    try {
        const userId = req.user?.userId;
        const tests = await database_1.prisma.codingTest.findMany({
